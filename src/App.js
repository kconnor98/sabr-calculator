...
const App = () => {
  ...
  useEffect(() => { const initFirebase = async () => { try { const app = initializeApp(firebaseConfig); const firestoreDb = getFirestore(app); const firebaseAuth = getAuth(app); setDb(firestoreDb); setAuth(firebaseAuth); onAuthStateChanged(firebaseAuth, async (user) => { if (user) { setUserId(user.uid); } else { try { if (initialAuthToken) { await signInWithCustomToken(firebaseAuth, initialAuthToken); } else { await signInAnonymously(firebaseAuth); } setUserId(firebaseAuth.currentUser?.uid || crypto.randomUUID()); } catch (error) { console.error('Firebase Auth Error:', error); setUserId(crypto.randomUUID()); } } setIsAuthReady(true); }); } catch (error) { console.error('Firebase Initialization Error:', error); } }; initFirebase(); }, []);
  useEffect(() => { if (!db || !isAuthReady) return; const suggestionsCollectionRef = collection(db, `artifacts/${appId}/public/data/suggestions`); const unsubscribe = onSnapshot(suggestionsCollectionRef, (snapshot) => { const fetchedSuggestions = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data(), timestamp: doc.data().timestamp ? doc.data().timestamp.toDate() : null })); setSuggestions(fetchedSuggestions.sort((a, b) => b.timestamp - a.timestamp)); }, (error) => { console.error("Error fetching suggestions: ", error); }); return () => unsubscribe(); }, [db, isAuthReady, appId]);
  useEffect(() => { if (!db || !userId) return; const institutionalDataDocRef = doc(db, `artifacts/${appId}/users/${userId}/institutionalData/customValues`); const unsubscribe = onSnapshot(institutionalDataDocRef, (docSnap) => { if (docSnap.exists()) { const data = docSnap.data(); setInstitutionalData({ ci100Tolerance: data.ci100Tolerance || '1.2', customTable: data.customTable && Array.isArray(data.customTable) && data.customTable.length > 0 ? data.customTable : [{ ptv: '', r50p: '', r50v: '', d2cmp: '', d2cmv: '' }] }); } }, (error) => { console.error("Error fetching institutional data: ", error); }); return () => unsubscribe(); }, [db, userId, appId]);
